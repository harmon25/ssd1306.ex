defmodule SSD1306.Commands do
  ...
  def reset!(gpio_pid) do
    with :ok <- ElixirALE.GPIO.write(gpio_pid, 1),
         :ok <- :timer.sleep(1),
  ...
  defp send_data(pid, << msb::integer-size(8), lsb::integer-size(8) >>), do: ElixirALE.I2C.write(pid, <<@data_register, msb, lsb>>)
  defp send_command(pid, byte), do: ElixirALE.I2C.write(pid, <<@control_register, byte>>)
